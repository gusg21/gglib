...
static lu_mem traversetable(global_State *g, Table *h) {
    ...
    if (mode && ttisstring(mode) && /* is there a weak mode? */
        (cast_void(weakkey = strchr(svalue(mode), 'k')), cast_void(weakvalue = strchr(svalue(mode), 'v')),
...
static void incstep(lua_State *L, global_State *g) {
    ...
    l_mem stepsize = (g->gcstepsize <= log2maxs(l_mem)) ? ((cast(l_mem, 1) << g->gcstepsize) / WORK2MEM) * stepmul
