...
#define l_checkmodep(m) \
    ((m[0] == 'r' || m[0] == 'w') && (m[1] == '\0' || ((m[1] == 'b' || m[1] == 't') && m[2] == '\0')))
#endif
...
static int g_write(lua_State *L, FILE *f, int arg) {
    ...
    for (; nargs--; arg++) {
        if (lua_type(L, arg) == LUA_TNUMBER) {
            ...
            int len = lua_isinteger(L, arg) ? fprintf(f, LUA_INTEGER_FMT, (LUAI_UACINT)lua_tointeger(L, arg))
...
*/
static const luaL_Reg meth[] = {{"read", f_read}, {"write", f_write}, {"lines", f_lines},     {"flush", f_flush},
